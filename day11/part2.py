...
def is_valid(state):
    ...
    for i in range(0, 14, 2):
        if not state[i] == state[i + 1] and any(
                state[j] == state[i] for j in range(1, 14, 2)):
            return False
    return True
...
# Takes about 8.5 min on my computer, could definitely be optimized
with open("input") as inf, open("part2.out", "w+") as outf:
    target = (4,) * 15
    start = (1, 1, 1, 1, 1, 1, 3, 2, 3, 2, 3, 2, 3, 2, 1)
